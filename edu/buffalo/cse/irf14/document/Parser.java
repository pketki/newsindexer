...
public class Parser {
	...
	public static Document parse(String filename) throws ParserException {
		...
		Pattern month = Pattern.compile("\\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\\D*");
			...
			while (reader.hasNextLine()) {
				...
				if (document.getField(FieldNames.TITLE) == null) {
					...
					if (reader.findWithinHorizon("<AUTHOR>", 200) != null) {
						...
						String[] authorInfo = reader.nextLine().split(",|</AUTHOR>");
					...
					//if a file is unformatted and has no date put everything in content
					if (!reader.hasNext()) {
